...
AS numbered_rows WHERE row_num % 2 = 1;
SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY WORKER_ID) AS row_num FROM Worker) 
AS numbered_rows WHERE row_num % 2 = 0;
...
SELECT MIN(SALARY) 
FROM (SELECT DISTINCT SALARY FROM Worker ORDER BY SALARY DESC LIMIT 5) 
AS top5salaries;
SELECT SALARY, GROUP_CONCAT(FIRST_NAME) AS EMPLOYEES
